...
def normcompliance_modifiedAndrade(freq_n, T_h):

    tau_n=1./(2*np.pi*freq_n)
...
def normcompliance_Mc11(freq_n):
...
def normcompliance_YT16(freq_n, T_h):
...
def plot_res_fit(df_master, df_GMaxw, units):
    ...
    modul = df_master.modul
    stor = '{}_stor'.format(modul)
    dstor = '{}_stor_delta'.format(modul)
    loss = '{}_loss'.format(modul)
    dloss = '{}_loss_delta'.format(modul)
    fig, (ax1, ax2) = plt.subplots(1,2, figsize=(8,0.75*4))
    # df_master.plot(x='f', y=[drelax], ax=ax1, logx=True, logy=False,
    #     color=['gray'], ls='', marker='o', markersize=3,
    #     label=["{}(filter)".format(modul)])
    # df_GMaxw.plot(x='f', y=[drelax], ax=ax1, label=['Prony fit'],
    #     logx=True, logy=False, ls='-', lw=2, color=['r'])
    df_master.plot(x='f', y=[dstor], label=["{}'(filter)".format(modul)],
        ax=ax1, logx=True, logy=False, color=['C0'], 
        alpha=0.5, ls='', marker='o', markersize=3)
    df_master.plot(x='f', y=[dloss], label=["{}''(filter)".format(modul)],
        ax=ax2, logx=True, logy=False, color=['C1'], 
        alpha=0.5, ls='', marker='o', markersize=3)
    df_GMaxw.plot(x='f', y=[dstor], label=["Prony fit"],
        ax=ax1, logx=True, logy=False, ls='-', lw=2, color=['C0'])
    df_GMaxw.plot(x='f', y=[dloss], label=["Prony fit"],
        ax=ax2, logx=True, logy=False, ls='-', lw=2, color=['C1'])
    ax1.set_xlabel('Frequency (Hz)')
    ax1.set_ylabel('Storage modulus ({})'.format(units[stor]))
    ax2.set_xlabel('Frequency (Hz)')
...
def plot_optfitresidual(df_master, df_masterlowTh, dict_prony, dict_prony_lowTh, N, units):
    ...
    df_master['G_stor_delta']=df_master['G_stor']-df_masterlowTh['G_stor']
    df_master['G_loss_delta']=df_master['G_loss']-df_masterlowTh['G_loss']
    df_GMaxw = visco.prony.calc_GMaxw(**dict_prony[N])
    df_GMaxwold = visco.prony.calc_GMaxw(**dict_prony_lowTh[N])
    df_GMaxw['G_stor_delta']=df_GMaxw['G_stor']-df_GMaxwold['G_stor']
    df_GMaxw['G_loss_delta']=df_GMaxw['G_loss']-df_GMaxwold['G_loss']
...
def plot_normalised_modulus(df_master,dict_prony,N,units):
    df_master['G_relax']=np.sqrt(df_master['G_stor']**2.+df_master['G_loss']**2.)
    df_GMaxw = visco.prony.calc_GMaxw(**dict_prony[N])
    df_GMaxw['G_relax']=np.sqrt(df_GMaxw['G_stor']**2.+df_GMaxw['G_loss']**2.)
    
    modul = df_GMaxw.modul
    relax = '{}_relax'.format(modul)
    fig, ax1 = plt.subplots(figsize=(4,0.75*4))
    df_master.plot(x='f', y=[relax], ax=ax1, logx=True, logy=False, 
        color=['gray'], ls='', marker='o', markersize=3,
        label=["{}(filter)".format(modul)])
    df_GMaxw.plot(x='f', y=[relax], ax=ax1, label=['Prony fit'],
        logx=True, logy=False, ls='-', lw=2, color=['r'])
    ax1.set_xlabel('Frequency (Hz)')
    ax1.set_ylabel('Relaxation modulus ({})'.format(units[relax])) 
    ax1.legend()
    plt.savefig(os.path.join(f_plot, f"GMX_fit_relaxation_modulus.jpg"), dpi=300)
...
def plot_param(prony_list, labels=None):
    ...
    xticklabels = [("{:.2e}".format(a)) for a in df_bar.index.tolist()]
    ax1.set_xticklabels(xticklabels)
    ax1.set_xlabel(r'$\tau_i$')
    ax1.set_ylabel(r'$\alpha_i$')
    ax1.grid(False)
    ax1.legend()
    plt.savefig(os.path.join(f_plot, f"GMX_fit_prony_params.jpg"), dpi=300)
...
for i in range(len(Th_input)):
    ...
    freq_input=np.logspace(minfreq,maxfreq,nx)
    output_YT16=np.zeros((nx,3))
    output_An=np.zeros((nx,3))
    output_Mc11=np.zeros((nx,3))

    k=0
    for j in range(len(freq_input)):
        J1_YT16, J2_YT16 = normcompliance_YT16(freq_input[j], Th_input[i])
        J1_Mc11, J2_Mc11 = normcompliance_Mc11(freq_input[j])
        J1_An, J2_An = normcompliance_modifiedAndrade(freq_input[j], Th_input[i])
        output_YT16[k,0]=freq_input[j]
        output_YT16[k,1]=J1_YT16/(np.sqrt((J1_YT16**2.)+(J2_YT16**2.))**2.)
        output_YT16[k,2]=J2_YT16/(np.sqrt((J1_YT16**2.)+(J2_YT16**2.))**2.)
        output_An[k,0]=freq_input[j]
        output_An[k,1]=J1_An/(np.sqrt((J1_An**2.)+(J2_An**2.))**2.)
        output_An[k,2]=J2_An/(np.sqrt((J1_An**2.)+(J2_An**2.))**2.)
        output_Mc11[k,0]=freq_input[j]
        output_Mc11[k,1]=J1_Mc11/(np.sqrt((J1_Mc11**2.)+(J2_Mc11**2.))**2.)
        output_Mc11[k,2]=J2_Mc11/(np.sqrt((J1_Mc11**2.)+(J2_Mc11**2.))**2.)
        k=k+1
            
    # header = '{0:^7s},{1:^7s}\n{2:^7s},{3:^7s}'.format('t', 'G_relax', 's', 'GPa')
    header = '{0:^7s},{1:^7s},{2:^7s}\n{3:^7s},{4:^7s},{5:^7s}'.format('f', 'G_stor', 'G_loss', 'Hz', 'GPa', 'GPa')
    np.savetxt(os.path.join(f_plot, f"output_YT16_Th"+repr(round(Th_input[i],2))+".csv"),output_YT16, fmt=('%5.4e','%5.4e','%5.4e'), comments='', header=header,delimiter=",")
    np.savetxt(os.path.join(f_plot, f"output_Mc11.csv"),output_Mc11, fmt=('%5.4e','%5.4e','%5.4e'), comments='', header=header,delimiter=",")
    ...
    # Plot normalised modulus (M1 and M2)
    plot_normalised_modulus(df_master,pronyout,N_opt,units)
